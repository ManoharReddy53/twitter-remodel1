...
def register(request):
    if request.method=="POST":
        ...
        if not isSpl(p):
            d={'warn':"password does not contains special characters "}
            return render(request,'register.html',d)
...
def profile(request):
    ...
    'c':request.user.first_name,
    'd':request.user.last_name,
    }
    print(di)
...
def Delete(request,rid):
    a=tweeter.objects.get(id=rid)
    a.delete()
    return redirect('home')
def view(request,rid):
    a=tweeter.objects.get(id=rid)
    return render(request,"view.html",{'post':a})